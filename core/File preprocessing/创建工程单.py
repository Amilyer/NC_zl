...
def insert_image_to_sheet(ws, image_path, temp_image_paths, target_range="A4:M12", custom_width=1062, custom_height=800):
    ...
    try:
        ...
        if custom_width is not None and custom_height is not None:
            ...
            target_w = int(total_width)
            target_h = int(total_height)

        # 计算缩放比例，保持长宽比
        width_ratio = target_w / original_width
        height_ratio = target_h / original_height
        ...
        # 缩放原图
        resized_img = img.resize((new_w, new_h), PillowImage.Resampling.LANCZOS)
        
        # 创建白色背景画布
        final_img = PillowImage.new("RGB", (target_w, target_h), (255, 255, 255))
